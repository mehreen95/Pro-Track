...
import { useGetDashboardStatsQuery } from '../redux/slices/api/projectApiSlice';
...
const UserTable = ({ users }) => {
  const TableHeader = () => (
    <thead className='border-b border-gray-300 '>
      <tr className='text-black  text-left'>
        ...
        <th className='py-2 w-32 pl-2'>Created At</th>
      </tr>
    </thead>
  );
  ...
  const TableRow = ({ user }) => (
    <tr className='border-b border-gray-200 text-gray-600 hover:bg-gray-400/10'>
      <td className='py-2'>
        <div className='flex items-center gap-3'>
          <div className='w-9 h-9 rounded-full text-white flex items-center justify-center text-sm bg-violet-700'>
            <span className=' w-10 text-center'>{getInitials(user?.name)}</span>
          </div>
      ...
      <td className='py-2 text-xs pl-4'>{moment(user?.createdAt).fromNow()}</td>
    </tr>
  );
  return (
