...
export const createTask = async (req, res) => {
  try {
    ...
    text =
      text +
      ` The project priority is set a ${priority} priority, so check and act accordingly. The project date is ${new Date(date).toDateString()}. Thank you!!!`;
...
export const createSubTask = async (req, res) => {
  try {
    ...
    res
      .status(200).json({ status: true, message: "Sub-Project added successfully." });
  } catch (error) {
    console.log(error);
    return res.status(400).json({ status: false, message: error.message });
  }
